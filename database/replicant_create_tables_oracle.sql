...
-- last update 2018.1.18
    ...
    CREATE TABLE REP_TAG(	 
	    ...
	    CREATION_DATE		TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL,
	    CONSTRAINT REP_TAG_PK PRIMARY KEY (TAG_ID)
    );
   
    CREATE UNIQUE INDEX REP_TAG_IDX1 ON REP_TAG( TAG_NAME ) ;	
    CREATE INDEX REP_TAG_IDX2 ON REP_TAG (CREATION_DATE) ;
   
    CREATE TABLE REP_OBJECT_TAG(	 
    		TAG_ID					INTEGER NOT NULL,	
		OBJECT_TYPE			INTEGER NOT NULL,
		OBJECT_ID				INTEGER NOT NULL,
   		CREATION_DATE		TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL
	...
	CREATE INDEX REP_OBJECT_TAG_IDX1 ON REP_OBJECT_TAG (OBJECT_ID, TAG_ID, OBJECT_TYPE) ;
    CREATE INDEX REP_OBJECT_TAG_IDX2 ON REP_OBJECT_TAG (TAG_ID) ;
    CREATE INDEX REP_OBJECT_TAG_IDX3 ON REP_OBJECT_TAG (OBJECT_TYPE, OBJECT_ID, CREATION_DATE, TAG_ID) ;
    CREATE INDEX REP_OBJECT_TAG_IDX4 ON REP_OBJECT_TAG (OBJECT_TYPE, TAG_ID) ;
  ...
  CREATE TABLE REP_PAGE_VERSION
   (	
	...
	CREATE INDEX REP_CODESET_IDX5 ON REP_CODESET (META_CODE) ;
