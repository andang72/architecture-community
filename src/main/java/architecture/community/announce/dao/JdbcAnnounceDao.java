...
public class JdbcAnnounceDao extends ExtendedJdbcDaoSupport implements AnnounceDao {
	...
	private String announcePropertyTableName = "AC_ANNOUNCE_PROPERTY";
	...
	public Announce load(long announceId) throws AnnounceNotFoundException {
		try {
		    Announce announce = getExtendedJdbcTemplate().queryForObject(
			    getBoundSql("COMMUNITY_CS.SELECT_ANNOUNCE_BY_ID").getSql(), announceMapper,
	    ...
	    public void update(Announce annoucne) {
			...
			getExtendedJdbcTemplate().update(getBoundSql("COMMUNITY_CS.UPDATE_ANNOUNCE").getSql(),
	    ...
	    public void insert(Announce announce) {
			...
			getExtendedJdbcTemplate().update(getBoundSql("COMMUNITY_CS.INSERT_ANNOUNCE").getSql(),
	    ...
	    public void delete(Announce annoucne) {
			getJdbcTemplate().update(getBoundSql("COMMUNITY_CS.DELETE_ANNOUNCE").getSql(),
				new SqlParameterValue(Types.NUMERIC, annoucne.getAnnounceId()));
	    ...
	    public List<Long> getAnnounceIdsForUser(long userId) {
			...
			getBoundSql("COMMUNITY_CS.SELECT_ANNOUNCE_IDS_BY_USER_ID").getSql(), new RowMapper<Long>() {
		...
		return getExtendedJdbcTemplate().query(
			getBoundSql("COMMUNITY_CS.SELECT_ANNOUNCE_IDS_BY_OBJECT_TYPE_AND_OBJECT_ID").getSql(),
			new RowMapper<Long>() {
			    public Long mapRow(ResultSet rs, int rowNum) throws SQLException {
				return rs.getLong(1);
			    }
		...
		return getExtendedJdbcTemplate().query(getBoundSql("COMMUNITY_CS.SELECT_ALL_ANNOUNCE_IDS").getSql(),
			new RowMapper<Long>() {
			    public Long mapRow(ResultSet rs, int rowNum) throws SQLException {
				return rs.getLong(1);
			    }
		...
		return getExtendedJdbcTemplate().queryForObject(
			getBoundSql("COMMUNITY_CS.COUNT_ANNOUNCE_BY_OBJECT_TYPE_AND_OBJECT_ID").getSql(),
			Integer.class,
			new SqlParameterValue(Types.NUMERIC, objectType), new SqlParameterValue(Types.NUMERIC, objectId)
			);
	    }
		...
		return getExtendedJdbcTemplate().queryForObject(
			getBoundSql("COMMUNITY_CS.COUNT_ANNOUNCE_BY_OBJECT_TYPE_AND_OBJECT_ID_AND_END_DATE").getSql(),
			Integer.class,
		...
		return getExtendedJdbcTemplate().queryForObject(
			getBoundSql("COMMUNITY_CS.COUNT_ANNOUNCE_BY_OBJECT_TYPE_AND_OBJECT_ID_AND_START_DATE_AND_END_DATE").getSql(),
			Integer.class,
			new SqlParameterValue(Types.NUMERIC, objectType), new SqlParameterValue(Types.NUMERIC, objectId),
